...
(function () {
  ...
  function intervalsFromSuffix(suffix, baseQuality) {
    ...
    if (/b5/i.test(s) && !intervals.includes(6)) {
      const idx = intervals.indexOf(7); if (idx >= 0) intervals[idx] = 6;
    }
    if (/#5/i.test(s)) { const idx = intervals.indexOf(7); if (idx >= 0) intervals[idx] = 8; }
    if (/b9/i.test(s)) { const idx = intervals.indexOf(14); if (idx >= 0) intervals[idx] = 13; else intervals.push(13); }
    if (/#9/i.test(s)) { const idx = intervals.indexOf(14); if (idx >= 0) intervals[idx] = 15; else intervals.push(15); }
    if (/#11/i.test(s)) { const idx = intervals.indexOf(17); if (idx >= 0) intervals[idx] = 18; else intervals.push(18); }
    if (/b13/i.test(s)) { const idx = intervals.indexOf(21); if (idx >= 0) intervals[idx] = 20; else intervals.push(20); }
  ...
  function buildChord(tokenStr, key, octave) {
    ...
    const suffixLabel = (resolved.suffix || '')
      ...
      .replace(/^maj/i, 'maj')
      .replace(/^o|^°|^dim/i, '');
    const label = rootName + qualityLabel + suffixLabel + (resolved.slashBass ? '/' + resolved.slashBass : '');
  ...
  function parseSource(src) {
    const directives = { kind: 'chord', key: 'C', tempo: 96, inst: 'piano', beats: 4, steps: 16, octave: 3 };
    const errors = [];
  ...
  const DRUM_ALIASES = {
    bd: 'bd', kick: 'bd', k: 'bd',
    sd: 'sd', snare: 'sd', sn: 'sd', s: 'sd',
    hh: 'hh', hat: 'hh', ch: 'hh', h: 'hh',
    oh: 'oh', open: 'oh',
    cp: 'cp', clap: 'cp',
    cy: 'cy', cym: 'cy', cymbal: 'cy',
    rd: 'rd', ride: 'rd',
    tm: 'tm', tom: 'tm', lt: 'tm', mt: 'tm', ht: 'tm',
    rs: 'rs', rim: 'rs',
  };
  const DRUM_DISPLAY = {
    bd: 'kick', sd: 'snare', hh: 'hat', oh: 'open hat',
    cp: 'clap', cy: 'cymbal', rd: 'ride', tm: 'tom', rs: 'rim',
  };
  ...
  function parseDrumSource(src, directives) {
    ...
    for (let li = 0; li < lines.length; li++) {
      ...
      if (!drum) {
        errors.push({ line: li + 1, token: name, msg: `unknown drum "${name}" (try bd, sd, hh, oh, cp, cy, rd, tm, rs)` });
        continue;
      }
      ...
      if (tokenMode) {
        ...
        for (const tok of tokens) {
          let isRest = false;
          let core = tok;
          if (core.startsWith('~')) { isRest = true; core = core.slice(1); }
          else if (/^r/i.test(core) && core.length > 1) { isRest = true; core = core.slice(1); }
    ...
    for (const lane of lanesRaw) {
      if (!mergedMap[lane.drum]) mergedMap[lane.drum] = { drum: lane.drum, events: [...lane.events] };
    ...
    const orderedLanes = DRUM_ORDER
      .filter((d) => lanesViz.find((l) => l.drum === d))
      .map((d) => lanesViz.find((l) => l.drum === d))
      .concat(lanesViz.filter((l) => !DRUM_ORDER.includes(l.drum)));

    return {
      kind: 'drums',
      directives, lanes: orderedLanes, events: renderedEvents,
      totalBeats, totalSec: totalBeats * beatSec, errors,
      chords: [],
  ...
  function makeSynth(Tone, kind) {
    ...
    if (kind === 'bass') {
      const s = new Tone.PolySynth(Tone.MonoSynth, {
        ...
        filterEnvelope: { attack: 0.01, decay: 0.2, sustain: 0.3, release: 0.6, baseFrequency: 80, octaves: 2.6 },
      });
      s.volume.value = -8;
      return s;
    }
  ...
  function makeDrumKit(Tone) {
    ...
    return {
      trigger: t,
      connect(node) { all.forEach((s) => s.connect(node)); return this; },
      toDestination() { all.forEach((s) => s.toDestination()); return this; },
    };
  }
  ...
  async function renderToBuffer(parsed, opts = {}) {
    ...
    const buffer = await Tone.Offline(async ({ transport }) => {
      const reverb = new Tone.Reverb({ decay: 2.2, wet: inst === 'guitar' ? 0.14 : inst === 'piano' ? 0.14 : 0.18 });
      ...
      for (const ev of parsed.events) {
        ...
        if (inst === 'guitar' && ev.chord.notesMidi[0]) {
          synth.triggerAttackRelease(midiToName(ev.chord.notesMidi[0] - 12), dur, ev.secStart + 0.006, bassVel * 0.85);
        }
        ev.chord.noteNames.forEach((n, idx) => {
          synth.triggerAttackRelease(n, dur, ev.secStart + (idx + 1) * stagger, chordVel);
        });
      }
    }, totalSec, 2, Tone.getContext().sampleRate);
  ...
  function computeGuitarFingering(chord) {
    ...
    for (let baseFret = 0; baseFret <= 9; baseFret++) {
      ...
      for (let s = 0; s < 6; s++) {
        ...
        if (candidates.length === 0) { positions[s] = null; mutedCount++; continue; }

        let pick;
        if (!hasBass) {
          const bm = candidates.find((f) => ((open + f) % 12 + 12) % 12 === bassPc);
          if (bm != null) { pick = bm; hasBass = true; bassString = s; }
      ...
      if (span > 4) continue;
      let firstSounded = -1;
      for (let s = 0; s < 6; s++) if (positions[s] != null) { firstSounded = s; break; }

      const score = mutedCount * 3
                  + baseFret * 0.3
                  + (hasBass ? 0 : 6)
  ...
  function drawPianoRoll(canvas, parsed, opts = {}) {
    ...
    for (const ev of parsed.events) {
      ...
      for (const m of all) { if (m < lo) lo = m; if (m > hi) hi = m; }
    }
  ...
  function playBuffer(buffer, onEnded) {
    ...
    return {
      ...
      duration: ab && ab.duration ? ab.duration : 0,
    };
  }
